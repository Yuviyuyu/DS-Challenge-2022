...
# userUrl = "http://www5.austlii.edu.au/au/legis/nsw/consol_act/capva2007347/"
browser.get(userUrl)
browser2 = webdriver.Chrome(executable_path=r"C:\Users\ysenz\chromedriver.exe") # Path to chrome driver
browser2.get('https://itcodefair.cdu.edu.au/data-science-challenge/')
driver = webdriver.Chrome(executable_path=r"C:\Users\ysenz\chromedriver.exe") # Path to chrome driver
...
# -------------- Find internal references and put into a list -------------- #  
int = browser.find_elements_by_xpath("//pre/a[@href]")
for element in int:
    links = element.get_attribute("href")
    internalUrls.append(links)
# print(internalUrls)

# -------------- Find external references and put into a list -------------- #  
ext = browser.find_elements_by_xpath("//a[contains(text(), 'Act') or contains(text(), 'ACT')]")
for element in ext:
    text = element.text
...
# print(sections)
 # -------------- Find subsections -------------- #
    ...
    if (check_exists_by_xpath("//blockquote") == True):
        url2 = browser.find_elements_by_xpath("//blockquote/a[@href]")
        for element in url2:
            links = element.get_attribute("href")
            internalUrls.append(links)
    else:
        url1 = browser.find_elements_by_xpath("//p/a[@href]")
        for element in url1:
            links = element.get_attribute("href")
            internalUrls.append(links)
# print(internalUrls)

  # -------------- Check external cross references -------------- # 
    if (check_exists_by_xpath("//blockquote") == True):
        url4 = browser.find_elements_by_xpath("//blockquote/a[contains(text(), 'Act') or contains(text(), 'ACT')]")
        for element in url4:
            text = element.text
            externalUrls.append(text)
    else:
        url3 = browser.find_elements_by_xpath("//p/a[contains(text(), 'Act') or contains(text(), 'ACT')]")
        for element in url3:
            text = element.text
            externalUrls.append(text)
# print(externalUrls)
...
def countWords(listName):
    listName = ''.join(listName).replace("--", " ").replace("\n", " ").translate(str.maketrans('','',string.punctuation))
    listName_count = len(listName.split())
    return listName_count

# -------------- Task 1 to 3 Word count of a legislative text  -------------- #    
totalWords = countWords(firstPage) + countWords(sections)
# print(totalWords)

num_sec = countWords(sections)
print("Task 2. CHAPTER Word Count: ", countWords(chapters) + num_sec, "Part", countWords(parts)+num_sec, "DIVISIONS", countWords(divisions)+num_sec, "subdivision", countWords(subdivisions)+num_sec, "Sections", num_sec)
print("Task 3. Number of Sections: ", len(sections)) 
print("Task 3. Number of Subsections: ", len(subsections))

# -------------- Task 4 to 5 Enactment Year and Years/Frequencies of Amendents  -------------- # 
# -------------- get first page URL from all availabe states -------------- #  
u = browser2.find_elements(By.XPATH, "//div[@class='elementor-widget-container']//table//tbody//tr//td//p//a")
for url in u:
    url = url.get_attribute('innerHTML')
    allLawUrls.append(url)
# print(allLawUrls)
# -------------- get enactment year -------------- # 
print("Task 4. Legislation Enactment Year: ")
for u in allLawUrls:
    driver.get(u)
...
# -------------- get endnotes URL -------------- #         
if (check_exists_by_xpath("//a[contains(@href,'endnotes')]") == True):    
    endnotes = browser.find_element_by_xpath("//a[contains(@href,'endnotes')]").get_attribute("href")
    browser.get(endnotes)
else:
    print("No endnotes to find amendent hitosry!") 
                

if 'nt' in userUrl:
    print("Task 5. Years and count the frequency of legislation amendments: ")
    driver.get(l)
    ...
    print("Task 5. Years and count the frequency of legislation amendments: ")
    b_list= browser.find_elements(By.XPATH, "html//body//p//font//b")
    for b in b_list:
        b=b.text
...
firstPageSentences = ''.join(firstPage).replace("--", " ").replace("\n", " ")
firstPageWords = firstPageSentences.translate(str.maketrans('','',string.punctuation))
totalWords = len(firstPageWords.split()) + countWords(sections)
# print(totalWords)

totalSentences = len(sent_tokenize(firstPageSentences)) + len(sent_tokenize(sectionSentences))
...
NumofExternal = len(externalUrls) - 1
NumofInternal = len(internalUrls)
...
print("Task 9. Could not define any uncovered URLs. The defination of uncovered cross-references pattern is not clear.")
